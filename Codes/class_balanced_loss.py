...
def class_balanced_loss(x,  y, beta, num_classes = 2, set_indicator = 0, loss = torch.nn.functional.binary_cross_entropy):
    '''
    ...
    #turn scale shapes into same shape as x and y. Rows have the same values throughout
    scales = scales * one_hot
    scales = scales.sum(1)
    scales = scales.unsqueeze(1)
    scales = scales.repeat(1,num_classes)
    ...
    cb_loss = loss(x, one_hot, weight = scales)
