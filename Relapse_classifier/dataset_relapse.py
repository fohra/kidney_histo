...
class CustomDataset(Dataset):
    def __init__(self, spot_dir, num_cancer, num_benign, seed, include_edge = False, include_center=True, sample_train=False, sample_validation=False, prediction=False):
        ...
        if self.pred:
            self.num_non_relapse = 0 # not used in prediction
            self.num_relapse = 0
        else:
            self.num_non_relapse = len(self.spot_infos[self.spot_infos['relapse'] == False])
            self.num_relapse = len(self.spot_infos[self.spot_infos['relapse'] == True])
    ...
    def get_num_images(self):
        ...
        '''
        return [self.num_non_relapse, self.num_relapse]